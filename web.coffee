...
server.get "/problems", (req, res, next) ->
  req.ret = problems.toDictionary().select((x) ->
    title: x.key
  )
  next()
...
server.get "/scoreboard", (req, res, next) ->
  req.ret = (@t1 = db.Teams).find.sync(t1).select((x) ->
    ...
    problemsdone: problems.toDictionary().select((y) ->
      problem: y.key
      done: x.problemsdone.toDictionary().any((z) ->
        z.key is y.key
      )
    )
    score: x.problemsdone.toDictionary().select((y) ->
      problems[y.key].points
    ).sum()
    penalty: new Date(x.problemsdone.toDictionary().select((y) ->
      (y.value.getTime() - roundStart.getTime()) * (1.0 / problems[y.key].points)
    ).sum())
  )
...
server.get "/problems", (req, res, next) ->
  teaminfo = (@t1 = db.Teams).find.sync(@t1,
    teamname: req.session.teamname
  ).first()
  req.ret.forEach (x) ->
    x.done = teaminfo.problemsdone.toDictionary().any((y) ->
      y.key is x.title
    )
...
server.get "/problems/:p", (req, res, next) ->
  ...
  if problems[req.params.p].sample
    ...
    fs.readdir.sync(null, "problems/" + problems[req.params.p].folder + "/sample/after").forEach (x) ->
      if req.ret.sample.any((y) ->
        y.file is x
      )
        req.ret.sample.first((y) ->
          y.file is x
...
Sync ->
  problems = JSON.parseWithDate(fs.readFile.sync(null, "problems/index.json", "utf8")).first((x) ->
    x.start <= new Date() and x.end >= new Date()
  ).problems
  roundStart = JSON.parseWithDate(fs.readFile.sync(null, "problems/index.json", "utf8")).first((x) ->
    x.start <= new Date() and x.end >= new Date()
  ).start
